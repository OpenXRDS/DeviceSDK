...
mod tests {
    ...
    async fn test_server_run_multiple() {
        ...
        let server_handle = tokio::spawn(async move {
            server
                .set_root_dir(target_dir.as_path().to_str().unwrap())
                .start()
                .await;
        });
    ...
    async fn test_server_ftp_crud() {
        ...
        let ftp_payload_mkd = FtpPayload {
            command: FtpCommands::MKD,
            payload_name: "test_dir".to_string(),
            payload: None,
        };
        let ftp_payload_cwd = FtpPayload {
            command: FtpCommands::CWD,
            payload_name: "test_dir".to_string(),
            payload: None,
        };
        let ftp_payload_pwd = FtpPayload {
            command: FtpCommands::PWD,
            payload_name: "".to_string(),
            payload: None,
        };
        let ftp_payload_stor1 = FtpPayload {
            command: FtpCommands::STOR,
            payload_name: "test_file1.txt".to_string(),
            payload: Some("test1".as_bytes().to_vec()),
        };
        let ftp_payload_stor2 = FtpPayload {
            command: FtpCommands::STOR,
            payload_name: "test_file2.txt".to_string(),
            payload: Some("test2".as_bytes().to_vec()),
        };
        ...
        let ftp_payload_retr = FtpPayload {
            command: FtpCommands::RETR,
            payload_name: "test_file1.txt".to_string(),
            payload: None,
        };
        let ftp_payload_dele1 = FtpPayload {
            command: FtpCommands::DELE,
            payload_name: "test_file1.txt".to_string(),
            payload: None,
        };
        let ftp_payload_dele2 = FtpPayload {
            command: FtpCommands::DELE,
            payload_name: "test_file2.txt".to_string(),
            payload: None,
        };
        let ftp_payload_list = FtpPayload {
            command: FtpCommands::LIST,
            payload_name: "".to_string(),
            payload: None,
        };
        let ftp_payload_cdup = FtpPayload {
            command: FtpCommands::CDUP,
            payload_name: "".to_string(),
            payload: None,
        };
        let ftp_payload_rmd = FtpPayload {
            command: FtpCommands::RMD,
            payload_name: "./test_dir".to_string(),
            payload: None,
        };
