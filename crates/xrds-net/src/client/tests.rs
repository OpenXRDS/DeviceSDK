mod tests {
    ...
    impl VideoTrackHandler for CustomVideoProcessor {
        ...
        > {
            Box::pin(async move {
                println!(
                    "🎬 Custom video processor started for track: {}",
                    track.id()
                );
                ...
                while let Ok((rtp_packet, _attributes)) = track.read_rtp().await {
                    ...
                    if packet_count % 30 == 0 {
                        println!(
                            "📦 Packet #{}: payload={}B, total={}B, seq={}, timestamp={}",
                            packet_count,
                            payload_size,
                            total_packet_size,
                        ...
                        let avg_size = if packet_count > 0 {
                            total_bytes / packet_count
                        } else {
                            0
                        };
                        println!(
                            "📊 Stats: avg={}B, min={}B, max={}B, total_packets={}",
    ...
    async fn custom_audio_handler(
        ...
        while let Ok((rtp_packet, _)) = track.read_rtp().await {
            ...
            if packet_count % 50 == 0 {
                println!(
                    "📦 Audio Packet #{}: payload_size={}B, seq={}, timestamp={}",
                    packet_count,
    ...
    > {
        ...
        subscriber
            ...
            .map_err(|e| e.to_string())?;
        subscriber
            .set_debug_dir_path(DEFAULT_DEBUG_FILE_PATH)
            .await?;
    ...
    fn test_file_download1() {
        let client_builder = ClientBuilder::new();
        let client = client_builder.set_protocol(PROTOCOLS::FILE).build();
    ...
    async fn test_client_webrtc_send_video_file() {
        ...
        let (mut publisher, subscriber, _session_id, server_handle) =
            establish_complete_webrtc_connection(port)
                .await
        ...
        let received_file =
            std::fs::read(video_debug_file_path).expect("Failed to read received file");
        server_handle.abort();

        // Assertions
        assert!(is_valid_h264(&file), "Sent file is not valid H264");
        assert!(
            is_valid_h264(&received_file),
            "Received file is not valid H264"
        );
