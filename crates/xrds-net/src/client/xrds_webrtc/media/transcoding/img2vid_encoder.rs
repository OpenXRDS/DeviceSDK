    ...
    pub fn encode_video(
        ...
        for (i, jpeg_file) in jpeg_files.iter().enumerate() {
            ...
            if decoded_frame.width() != target_width || decoded_frame.height() != target_height {
                eprintln!(
                    "Warning: Frame {} has different dimensions ({}x{} vs {}x{}), scaling...",
                    jpeg_file,
                    decoded_frame.width(),
            ...
            let mut final_frame = convert_frame_format(
                &decoded_frame,
        ...
        log::info!(
            "Encoding complete. Total frames processed: {}",
            self.frame_count
        );
        // average encoding speed
        let elapsed = self.starting_time.elapsed().as_secs_f64();
        log::info!(
            "Total time: {:.2} seconds, Average FPS: {:.2}",
            elapsed,
