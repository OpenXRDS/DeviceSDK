...
impl Jpeg2H264Transcoder {
    pub fn new(width: u32, height: u32, fps: u32) -> Result<Self, Error> {
        ...
        println!(
            "🔧 Encoder settings: {}x{} @ {}fps, time_base={}/{}, GOP=30",
            width,
            height,
            fps,
            time_base.numerator(),
    ...
    fn decode_jpeg_bytes(&self, jpeg_bytes: &[u8]) -> Result<frame::Video, Error> {
        ...
        if jpeg_bytes.len() >= 2 && (jpeg_bytes[0] != 0xFF || jpeg_bytes[1] != 0xD8) {
            log::error!(
                "Invalid JPEG magic bytes: {:02X} {:02X}",
                jpeg_bytes[0],
                jpeg_bytes[1]
            );
            return Err(Error::InvalidData);
        ...
        let decoder_codec = decoder::find(codec::Id::MJPEG).ok_or(Error::DecoderNotFound)?;
     ...
     */
    pub fn transcode_jpeg_to_h264_packet(
        &mut self,
        jpeg_bytes: &[u8],
    ) -> Result<Vec<H264Packet>, Error> {
        ...
        while self.encoder.receive_packet(&mut h264_packet).is_ok() {
            if let Some(data) = h264_packet.data() {
                ...
                log::trace!(
                    "Generated H.264 packet: PTS={}, DTS={}, size={}, keyframe={}",
                    packet_pts,
                    packet_dts,
                    data.len(),
                    is_keyframe
        ...
        unsafe {
            ...
            if !coeffs.is_null() {
                let ret = sws_setColorspaceDetails(
                    sws_ctx,
                    coeffs,
                    1, // Input: JPEG full range (0-255)
                    coeffs,
                    0,       // Output: MPEG limited range (16-235)
                    0,       // Brightness
                    1 << 16, // Contrast
                    1 << 16, // Saturation
                );
    ...
    pub fn write_h264_packets_to_mp4(
        ...
        for (i, h264_pkt) in h264_packets.iter().enumerate() {
            ...
            if i < 5 {
                println!(
                    "📦 Packet {}: Original PTS={}, DTS={} -> Rescaled PTS={:?}, DTS={:?}",
                    i,
                    h264_pkt.pts,
                    h264_pkt.dts,
                    packet.pts(),
    ...
    pub fn flush_to_packets(&mut self) -> Result<Vec<H264Packet>, Error> {
        ...
        while self.encoder.receive_packet(&mut packet).is_ok() {
            if let Some(data) = packet.data() {
                ...
                log::trace!(
                    "Flush packet: PTS={}, DTS={}, size={}, keyframe={}",
                    pts,
                    dts,
                    data.len(),
                    packet.is_key()
...
async fn test_client_jpeg_to_h264() {
    ...
    let jpeg_files =
        get_jpeg_files("test_output/test_frame_%03d.jpg").expect("Failed to get JPEG files");
    ...
    let mut transcoder =
        Jpeg2H264Transcoder::new(1920, 1080, 30).expect("Failed to create transcoder");
    ...
    for (i, jpeg_file) in jpeg_files.iter().enumerate() {
        ...
        match std::fs::read(jpeg_file) {
            Ok(jpeg_data) => match transcoder.transcode_jpeg_to_h264_packet(&jpeg_data) {
                Ok(packets) => {
                    successful_frames += 1;
    ...
    println!(
        "📊 Processed {} frames, generated {} packets so far",
        successful_frames, packets_generated
    );
    ...
    match transcoder.flush_to_packets() {
        Ok(flush_packets) => {
            if !flush_packets.is_empty() {
                println!(
                    "🔄 Flushed {} additional packets from encoder",
                    flush_packets.len()
                );
    ...
    println!(
        "📦 Total H.264 packets: {} (from {} input frames)",
        all_h264_packets.len(),
        jpeg_files.len()
    );
    ...
    println!(
        "🎬 Reassigned timestamps: 0 to {}",
        all_h264_packets.len() - 1
    );
    ...
    match Jpeg2H264Transcoder::write_h264_packets_to_mp4(
        ...
        Ok(_) => {
            ...
            println!(
                "📹 Actual duration: {:.2}s ({} frames)",
                actual_duration,
                all_h264_packets.len()
            );
            println!(
                "📹 Expected duration: {:.2}s ({} frames)",
                expected_duration,
                jpeg_files.len()
            );
            ...
            println!(
                ...
                all_h264_packets.len() as f64 / 30.0
            );
            println!("   Time scale: 1000");
            println!(
                "   Expected duration in time units: {}",
                (all_h264_packets.len() as f64 / 30.0 * 1000.0) as i64
            );
