...
To run a compiled binary directly (e.g. a release build), use the wrapper instead, which applies the same environment:

```bash
./run-editor.sh
```

**Window resize stutter.** Dragging the window border causes the Vulkan swap chain to report "surface changed" on each resize event — a Vulkan/X11 limitation that Bevy recovers from by dropping the affected frames. It does not affect normal operation. Avoid slow edge-drags during screen recording or demos; use the window manager's keyboard shortcut (e.g. `Super+←/→`) to snap the window to a new size instead.
