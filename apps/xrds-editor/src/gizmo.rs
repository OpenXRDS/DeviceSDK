...
/// Draws world-space `XrdsText` nodes as egui text labels projected onto the
/// viewport.  `Text2d` uses Bevy's 2D pipeline which requires `Camera2d`; since
/// the editor viewport only has `Camera3d` we replicate the text as a 2D overlay
/// so nodes are visible and selectable while editing.
pub fn text3d_label_system(
    ...
    if editor_state.as_ref().map_or(false, |s| s.is_playing) {
        return;
    }
    ...
    for node in &session.document().nodes {
        let XrdsSceneNodePayload::Text(text_data) = &node.payload else { continue; };
        if !node.visible || text_data.text.is_empty() {
            continue;
        }

        let xrds_id = xrds::sdk::XrdsId::from(node.id);
        let Some(entity) = id_index.entity_of(xrds_id) else { continue; };
        let Ok(world_tf) = transforms.get(entity) else { continue; };

        let Ok(screen_pos) = camera.world_to_viewport(cam_tf, world_tf.translation()) else {
            continue;
        };

        let [r, g, b, a] = text_data.color;
        let color = xrds::editor::egui::Color32::from_rgba_unmultiplied(
            (r * 255.0) as u8,
            (g * 255.0) as u8,
            (b * 255.0) as u8,
            (a * 255.0) as u8,
        ...
        let align = match text_data.alignment {
            XrdsSceneTextAlignment::Left => xrds::editor::egui::Align2::LEFT_CENTER,
            XrdsSceneTextAlignment::Center => xrds::editor::egui::Align2::CENTER_CENTER,
            XrdsSceneTextAlignment::Right => xrds::editor::egui::Align2::RIGHT_CENTER,
        };

        let painter = ctx.layer_painter(xrds::editor::egui::LayerId::new(
            xrds::editor::egui::Order::Background,
            xrds::editor::egui::Id::new(("text3d_label", node.id.0)),
        ));

        painter.text(
            xrds::editor::egui::pos2(screen_pos.x, screen_pos.y),
            align,
            &text_data.text,
            xrds::editor::egui::FontId::proportional(text_data.font_size.clamp(8.0, 72.0)),
            color,
        );
    }
}
