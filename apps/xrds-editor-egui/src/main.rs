...
use gizmo::{
    floor_grid_system, interaction_zone_gizmo_system, light_rays_system,
...
impl XrdsApp for XrdsEditorApp {
    ...
    fn update(&mut self, ctx: &mut XrdsUpdateContext<'_>) {
        ...
        let (pl, dl, sl, al, extruded_color) = ctx
            .resource::<EditorState>()
            .map(|st| {
                (
        ...
        if let Some((id, color)) = extruded_color {
            ctx.set_extruded_text_color_for_node(xrds::sdk::XrdsId::from(id), color);
        }
