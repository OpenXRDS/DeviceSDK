...
use xrds::scene_graph::{
    ...
    XrdsSceneExtrudedText, XrdsSceneNodePayload, XrdsScenePlayerSpawn, XrdsSceneText,
    XrdsSceneTextAlignment, XrdsSceneTextAnchor, XrdsSceneTextureRef, XrdsXrBlendMode,
};
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                match &payload {
                    ...
                    XrdsSceneNodePayload::ExtrudedText(text) => {
                        extruded_text_section(ui, session, editor_state, selected_id, text);
                    }
...
fn text3d_section(
    ...
    egui::CollapsingHeader::new("Text 3D")
        ...
        .show(ui, |ui| {
            ...
            let mut anchor = text_node.anchor;
            let mut changed = false;
            ...
            // Anchor mode selector — discriminants for comparison.
            let anchor_disc = std::mem::discriminant(&anchor);
            ui.horizontal(|ui| {
                ui.label("Anchor");
                for (label, variant) in &[
                    ("World",    XrdsSceneTextAnchor::World),
                    ("Billboard",XrdsSceneTextAnchor::Billboard),
                    ("Head",     XrdsSceneTextAnchor::HeadLocked),
                    ("Body",     XrdsSceneTextAnchor::BodyLocked),
                    ("Pinned",   XrdsSceneTextAnchor::ComfortPinned { depth_m: match anchor { XrdsSceneTextAnchor::ComfortPinned { depth_m } => depth_m, _ => 1.0 } }),
                    ("Cylinder", XrdsSceneTextAnchor::Cylindrical   { radius_m: match anchor { XrdsSceneTextAnchor::Cylindrical { radius_m } => radius_m, _ => 1.0 } }),
                ] {
                    let selected = std::mem::discriminant(variant) == anchor_disc;
            ...
            // Sub-fields for variants with parameters.
            match &mut anchor {
                XrdsSceneTextAnchor::ComfortPinned { depth_m } => {
                    ui.horizontal(|ui| {
                        ui.label("Depth (m)");
                        if ui.add(egui::DragValue::new(depth_m).speed(0.05).range(0.1..=10.0)).changed() {
                            changed = true;
                        }
                    });
                }
                XrdsSceneTextAnchor::Cylindrical { radius_m } => {
                    ui.horizontal(|ui| {
                        ui.label("Radius (m)");
                        if ui.add(egui::DragValue::new(radius_m).speed(0.05).range(0.1..=10.0)).changed() {
                            changed = true;
                        }
                    });
                }
            ...
            if changed {
                let new_text = XrdsSceneText { text, font_size, color, alignment, anchor };
                let _ = session.session.edit(|doc| {
                    if let Some(n) = doc.node_mut(selected_id) {
...
fn extruded_text_section(
    ui: &mut egui::Ui,
    session: &mut EditorSession,
    editor_state: &mut EditorState,
    selected_id: xrds::scene_graph::XrdsSceneNodeId,
    text_node: &XrdsSceneExtrudedText,
) {
    egui::CollapsingHeader::new("Extruded Text")
        .default_open(true)
        .show(ui, |ui| {
            let mut text = text_node.text.clone();
            let mut font_size = text_node.font_size;
            let mut color = text_node.color;
            let mut depth = text_node.depth;
            let mut alignment = text_node.alignment;
            ...
            ui.horizontal(|ui| {
                ui.label("Depth");
                if ui
                    .add(egui::DragValue::new(&mut depth).speed(0.01).range(0.0..=5.0))
                    .changed()
                {
                    structural_changed = true;
                }
            ...
            if structural_changed || color_changed {
                let new_text = XrdsSceneExtrudedText { text, font_size, color, depth, alignment };
                let _ = session.session.edit(|doc| {
                    if let Some(n) = doc.node_mut(selected_id) {
                        if let XrdsSceneNodePayload::ExtrudedText(t) = &mut n.payload {
                            *t = new_text;
                        }
                    }
                });
                if structural_changed {
                    // Mesh must be regenerated — full entity despawn/respawn.
                    editor_state.needs_full_reimport = true;
                } else {
                    // Color-only: update StandardMaterial in-place to avoid the
                    // bevy_fontmesh deferred-command race condition that crashes
                    // when a reimport despawns TextMesh entities that update_text_meshes
                    // has already queued commands for in the same frame.
                    editor_state.pending_extruded_color = Some((selected_id, color));
                }
            }
        });
}
