...
export function Inspector({ snapshot, send }: Props) {
  ...
  return (
    <div className="inspector">
      ...
      {/* Transform — hidden for HudText (screen-space, no 3D world position) */}
      ...
      <PayloadSection key={node.id} node={node} send={send} isPlaying={snapshot.is_playing} />
    </div>
  );
}
...
function PayloadSection({ node, send, isPlaying }: { node: NodeInspector; send: (c: EditorCommand) => void; isPlaying: boolean }) {
  ...
  if (payload.type === "Cube" || payload.type === "Sphere" || payload.type === "Cylinder" ||
      payload.type === "Plane") {
    return <MaterialSection id={id} mat={payload.material} send={send} />;
  }
  ...
  if (payload.type === "Player") {
    return null; // Player has no editable fields beyond name and transform
  }
  if (payload.type === "PlayerAnchor") {
    return <PlayerAnchorSection id={id} p={payload} send={send} isPlaying={isPlaying} />;
  }
  return null;
}
...
function PlayerAnchorSection({ id, p, send, isPlaying }: { id: number; p: any; send: (c: EditorCommand) => void; isPlaying: boolean }) {
