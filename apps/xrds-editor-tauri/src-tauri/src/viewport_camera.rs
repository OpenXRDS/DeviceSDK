...
pub fn spawn_editor_camera(mut commands: Commands) {
    commands.spawn((
        Camera3d::default(),
        EditorCameraMarker,
        XrdsReceivesEnvironment, // opt in to SDK scene environment (fog, exposure, IBL)
    ));
}
...
pub fn orbit_camera_system(
    ...
    // In the Tauri setup the Bevy window is the viewport — no UI overlay,
    // so egui-style "wants pointer/keyboard" guards are always false.
    ...
    if is_fly {
        if right && !egui_wants_pointer && delta != Vec2::ZERO {
        ...
        if middle && shift && !egui_wants_pointer && delta != Vec2::ZERO {
