...
use crate::bridge::{
    EditorCommand, GltfClipDto, MaterialParamsDto, NodeInspectorDto, NodePayloadDto,
};
...
pub fn build_node_inspector(
    ...
    Some(NodeInspectorDto {
        ...
        scale: s,
        payload: build_payload_dto(&node.payload, id, gltf_clips),
        parent_kind,
    })
}
...
fn build_payload_dto(
    ...
    match payload {
        XrdsSceneNodePayload::Cube(c)       => NodePayloadDto::Cube     { material: mat_dto(&c.material) },
        XrdsSceneNodePayload::Sphere(c)     => NodePayloadDto::Sphere   { material: mat_dto(&c.material) },
        XrdsSceneNodePayload::Cylinder(c)   => NodePayloadDto::Cylinder { material: mat_dto(&c.material) },
        XrdsSceneNodePayload::Plane3D(c)    => NodePayloadDto::Plane    { material: mat_dto(&c.material) },
        XrdsSceneNodePayload::Tetrahedron(c)=> NodePayloadDto::Cube     { material: mat_dto(&c.material) }, // reuse Cube DTO for now
        ...
        XrdsSceneNodePayload::PlayerAnchor(a) =>
            NodePayloadDto::PlayerAnchor { fov_deg: a.fov_deg, is_initial: a.is_initial },
