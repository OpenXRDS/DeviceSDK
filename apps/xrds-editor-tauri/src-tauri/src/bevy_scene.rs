use std::sync::Arc;
use bevy::prelude::{App, IntoScheduleConfigs, PostUpdate, Update};
use xrds_runtime::{Runtime, RuntimeParameters, XrdsAPI, XrdsApp, XrdsUpdateContext, XrdsUpdateSystemSet};
use xrds_scene_graph::{
    XrdsEditorMetadata, XrdsSceneAmbientLight, XrdsSceneCamera, XrdsSceneCameraProjection,
...
use crate::viewport_camera::{EditorCameraState, apply_camera_selection_system, orbit_camera_system, spawn_editor_camera, EditorCameraMarker};
use crate::viewport_gizmo::{
    floor_grid_system, fov_overlay_system, interaction_zone_gizmo_system, light_rays_system,
    player_spawn_gizmo_system, transform_gizmo_system, update_selection_outline, GridGizmoGroup,
};
...
impl XrdsApp for XrdsEditorTauriApp {
    ...
    fn update(&mut self, ctx: &mut XrdsUpdateContext<'_>) {
        ...
        )).unwrap_or((false, vec![], vec![], None, None, None, None, None, None, None, None, None));
...
pub fn run_bevy_viewport(bridge: Arc<EditorBridge>) {
    // CARGO_MANIFEST_DIR = apps/xrds-editor-tauri/src-tauri
    ...
    Runtime::new(RuntimeParameters {
        app_name: "XRDS Editor — Viewport".to_owned(),
        run_on_any_thread: true,
        asset_path: Some(asset_path),
        // Allow loading GLB/GLTF files from arbitrary filesystem paths.
