...
pub fn main() {
    let runtime = Runtime::new(RuntimeParameters {
        app_name: "AtmosphericFog".to_owned(),
        ..Default::default()
    });
...
fn setup(
    ...
    commands.spawn((
        ...
        Transform::from_xyz(100.0, 100.0, 10.0).looking_at(Vec3::ZERO, Vec3::Y),
        cascade_shadow_config,
    ));

    commands.spawn(SceneRoot(asset_server.load(
        GltfAssetLabel::Scene(0).from_asset("models/Sponza/Sponza.gltf"),
    )));
    ...
    commands.spawn((
        Camera3d::default(),
        Transform::from_xyz(-5.0, 1.5, 0.0).looking_at(Vec3::new(10.0, 10.0, 0.0), Vec3::Y),
        DistanceFog {
