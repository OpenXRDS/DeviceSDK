...
> **[`xrds-net-crypto-consolidation.md`](xrds-net-crypto-consolidation.md)** —
...
- [x] No action taken, by decision. WebRTC stays desktop-only and out of the
      `xrds::net` intent-verb path — not part of this shipping effort.
...
- [x] Introduce finer-grained Cargo features in `xrds-net` if a future need
      arises to drop other individual protocols per platform. **Not needed** —
      the crypto consolidation (see
      [`xrds-net-crypto-consolidation.md`](xrds-net-crypto-consolidation.md))
      means every client-facing protocol (http/https/file/coap/mqtt/ftp/sftp/
      ws/wss/quic) is now functionally identical on Android and desktop; the
      *only* Android exclusion remains `ftp-server`, already handled below.
      Revisit only if a future protocol genuinely needs to differ per platform.
- [x] Any protocol marked `unsupported` returns a clear
      `NetError::Capability` at runtime on Android (never a link error, never a
      hang). **Re-scoped, not a live gap:** the only thing excluded on
      Android is the FTP *server* (`ftp-server` feature), which isn't reached
      through `XrdsNet`'s client intent verbs at all — it's a separate,
      explicit `XRNetServer::start()` API. With the feature off, that path
      logs a warning and no-ops rather than erroring (§2d), verified via unit
      tests compiled both with and without the feature. There is currently no
      client-facing intent verb that is unsupported specifically on Android
      (WebRTC is unsupported on *every* platform via intent verbs, by
      design — §2e), so the original "unsupported-on-Android" runtime check
      this item anticipated doesn't apply to anything today.

---

## Phase 4 — On-device verification (Quest 3/Pro)

- [x] Built a minimal networking smoke test (temporary code in
      `apps/xrds-app/src/main.rs`, since removed): one `request_async` (https)
      and one `open` (wss session), polled/logged from `setup()`/`update()`.
- [x] Deployed to a real Quest 3 (`adb install` + launch), confirmed both
      calls **completed on-device** (not just linked) via logcat:
      ```text
      [net-smoke] http DONE status=200 body_len=559
      [net-smoke] wss DONE recv 32 bytes: "Request served by 4d896d95b55478"
      ```
- [x] Confirmed no runtime crash from a missing TLS/crypto provider. This
      surfaced as a *real* bug during desktop test re-verification first
      (rustls's `CryptoProvider` was only ever installed ad hoc by one test
      file) — fixed properly with an automatic, idempotent installer
      (`common::ensure_rustls_crypto_provider`, called from both the WS and
      FTP connect paths) before ever reaching the device. No crash on Quest.
      Full detail, plus two other on-device-specific findings (missing
      `INTERNET` permission, `eprintln!` being invisible on Android) in
      [`xrds-net-crypto-consolidation.md`](xrds-net-crypto-consolidation.md).
...
- [x] Updated the platform matrix in
      [`README.md`](../../crates/xrds-net/README.md) and
      [`MANUAL.md`](../../crates/xrds-net/MANUAL.md) §12/§14 — Android is now ✅
      (was ✗/excluded), and the old per-protocol native-tls/curl caveats were
      replaced with the two-backend rustls+BoringSSL story during the crypto
      consolidation (see
      [`xrds-net-crypto-consolidation.md`](xrds-net-crypto-consolidation.md)
      Phase 5).
- [x] Checked `CLAUDE.md`'s platform table — no change needed. It already
      showed Meta Quest 3/Pro+ as ✓ for "SDK / runtime" at the component
      level (doesn't break out networking specifically), and that claim is
      still accurate now that networking genuinely works there.
- [x] Move this checklist to `docs/done/` — done as part of this update.
