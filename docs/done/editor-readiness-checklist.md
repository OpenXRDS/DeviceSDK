...
**The SDK basement is complete. GUI editor work can begin.**

Completed since initial baseline:

- Stable object identity via `XrdsId`
- Registry mapping via `XrdsRegistry`
- Canonical XRDS hierarchy index with parent/child id queries
- Common transform data via `TransformParams` (quaternion is authoritative; Euler preserved for authoring convenience)
- SDK-level color type via `XrdsColor`
- Scene nodes for camera, glTF asset, node, cube, cylinder, sphere, plane, tetrahedron, point light, directional light, spot light, ambient light, and **audio clip**
- XRDS-level spawn path through `XrdsAPI::spawn(&descriptor)` without exposing Bevy construction APIs
- Runtime editing support for transform, visibility, rename, reparent, duplicate, delete, material (full params, individual fields, **texture slots**), light parameters, camera projection, glTF source, and built-in primitive geometry
- Full inspector read API: `camera_projection`, `camera_look_at`, all four `*_light_params`, `gltf_source`, `material_params`, `material_textures`, `material_pbr_params`, `material_base_color`, `material_emissive`
- Asset catalog with four first-class kinds: `Gltf`, `Texture`, `EnvironmentMap`, `Audio`
- Public scene-document import/export round-tripping all types cleanly
- Naming aligned to application-level conventions (`roughness`, `affects_baked_lighting`, `UvParams`, `SamplerParams`)
- `entity_of_id` documented as expert escape hatch; every XRDS camera auto-assigned `SpatialListener`
- Integration tests covering import, live-edit, export, environment map round-trip, audio clip round-trip
...
- four first-class asset kinds: `Gltf` (scene models), `Texture` (material slots), `EnvironmentMap` (IBL/skybox, HDR/EXR/KTX2/DDS), `Audio` (MP3/OGG/WAV/FLAC)
- catalog-backed glTF references on scene nodes through `asset_id`, embedded fallback `asset_uri`, `scene_index`, and export policy
- document/session helpers for registering, ensuring, placing, retargeting, rebinding, renaming, and removing assets of all four kinds
...
- environment-referenced assets (IBL, skybox) excluded from `unused_asset_ids`; audio-referenced nodes drive audio asset usage tracking
- runtime import/export that preserves all asset kinds and round-trips them through XRDS scene documents
...
- [~] Review naming to keep the default API application-level and non-engine-shaped
  (done: `roughness`, `affects_baked_lighting`, `UvParams/SamplerParams`; remaining: `TransformParams` dual-rotation, `*Patch` types)
