...
impl TaaProc {
    pub fn new(graphics_instance: &GraphicsInstance) -> anyhow::Result<Self> {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: None,
            bind_group_layouts: &[
                &bind_group_layout, // Final color from framebuffer
                &bind_group_layout, // Previous final color from framebuffer
                &bind_group_layout, // Motion vector
            ],
        ...
        let fragment_descriptor = preprocessor
            .build(
                include_str!("../shader/postproc/taa.wgsl"),
                &defs,
                Some("taa.wgsl"),
            )
            .unwrap();
