...
impl<M: Message> WinitAppRunnerState<M> {
    fn redraw_requested(&mut self, event_loop: &ActiveEventLoop) {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            static RDR_CALL: AtomicU64 = AtomicU64::new(0);
            let n = RDR_CALL.fetch_add(1, Ordering::Relaxed);
            if n < 30 || n % 90 == 0 {
                // SAFETY: eprintln goes to logcat on Android
                eprintln!("[bevy_winit] redraw_requested call={} should_update={} ran_since_redraw={} lifecycle={:?}",
                    n, should_update, self.ran_update_since_last_redraw, self.lifecycle);
            }
        }

        // On Android/Quest, the Android window surface is destroyed when the VR compositor takes
        // over (Activity.onPause). RedrawRequested events never arrive (no surface), so
        // ran_update_since_last_redraw is never reset to false via the normal path. Reset it
        // here directly so every about_to_wait() call triggers app.update(); xrWaitFrame throttles
        // the actual frame rate to the display refresh rate.
        #[cfg(target_os = "android")]
            ...
            UpdateMode::Continuous => {
                ...
                cfg_if::cfg_if! {
                    ...
                    )))]
                    {
                        ...
                        // On Android/Quest, the Android window surface is destroyed when the VR
                        // compositor takes over the display (Activity.onPause). With ControlFlow::Wait,
                        // window.request_redraw() becomes a no-op and the loop stalls forever.
                        // Use Poll so about_to_wait() is called continuously; xrWaitFrame throttles the rate.
                        #[cfg(target_os = "android")]
        ...
        // On Android/Quest, keep requesting redraws even when the Activity is suspended
        // so the event loop wakes up and xrWaitFrame can run each frame.
        let suspend_blocks_redraw = self.lifecycle == AppLifecycle::Suspended;
        #[cfg(target_os = "android")]
        let suspend_blocks_redraw = false;
        if self.redraw_requested && !suspend_blocks_redraw {
            WINIT_WINDOWS.with_borrow(|winit_windows| {
    ...
    fn should_update(&self, update_mode: UpdateMode) -> bool {
        // On Android/Quest, the VR compositor suspends the NativeActivity (onPause) while
        // XR rendering is active. Keep updating in Continuous mode regardless of Android
        // lifecycle — xrWaitFrame throttles the frame rate instead.
        #[cfg(target_os = "android")]
        if matches!(update_mode, UpdateMode::Continuous) {
            return true;
        }
