...
fn main() {
        ...
        !cfg!(feature = "openssl")
    {
        ...
        // If the guessed (MSVC multi-config, e.g. "build/Debug") directory
        // doesn't exist, try the plain single-config CMake output directory
        // ("build/", no per-config subfolder) before giving up. This covers
        // cross-compiling to a non-MSVC target (e.g. Android via Ninja) from
        // an MSVC host, where get_boringssl_platform_output_path() guesses a
        // Visual-Studio-style subdirectory based on the *host* target_env
        // even though the actual CMake generator (forced to Ninja for the
        // Android/NDK build) is single-config and places libssl.a/libcrypto.a
        // directly under "build/".
        if !std::path::Path::new(&build_dir).is_dir() {
            let plain_build_dir = format!("{bssl_dir}/build");
            build_dir = if std::path::Path::new(&plain_build_dir).is_dir() {
                plain_build_dir
            } else {
                bssl_dir
            };
